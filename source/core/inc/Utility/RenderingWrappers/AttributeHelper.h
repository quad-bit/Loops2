...
namespace Core
{
    ...
    namespace Utility
    {
        ...
        struct VertexMetaData
        {
            ...
            Wrappers::VertexInputAttributeInfo* attribInfoList;
        };
        ...
        struct AttribPC : public AttribStructBase
        {
            ...
            AttribPC()
            {
                ...
                metaData.attribInfoList = new Wrappers::VertexInputAttributeInfo[metaData.attribCount];

                // position
                {
                    Wrappers::VertexInputAttributeInfo info = {};
                    info.binding = 0; // As a single vertex buffer is used per mesh
                    info.format = Enums::Format::R32G32B32_SFLOAT; //vec3 position
                    info.location = 0;
                ...
                //color
                {
                    Wrappers::VertexInputAttributeInfo info = {};
                    info.binding = 0; // As a single vertex buffer is used per mesh
                    info.format = Enums::Format::R32G32B32A32_SFLOAT; //uvec4 color
                    info.location = 1;
            ...
            template<typename T>
            void FillData(ECS::Components::Mesh* mesh);

            template<typename T>
            void FillData(ECS::Components::Mesh* mesh, const glm::vec4& color);
        };
        ...
        struct AttribPCN : public AttribStructBase
        {
            ...
            AttribPCN()
            {
                ...
                //Normal
                {
                    Wrappers::VertexInputAttributeInfo info = {};
                    info.binding = 0; // As a single vertex buffer is used per mesh
                    info.format = Enums::Format::R32G32B32_SFLOAT;  //uvec3 normal
                    info.location = 2;
...
template<typename T>
inline void Core::Utility::AttribPC::FillData(Core::ECS::Components::Mesh* mesh)
{
    T obj;
...
template<typename T>
inline void Core::Utility::AttribPC::FillData(Core::ECS::Components::Mesh* mesh, const glm::vec4& color)
{
    T obj;
...
template<typename T>
inline void Core::Utility::AttribPCN::FillData(Core::ECS::Components::Mesh* mesh)
{
    T obj;
...
template<typename T>
inline void Core::Utility::AttribPCN::FillData(Core::ECS::Components::Mesh* mesh, const glm::vec4& color)
{
    T obj;
    ...
    // not going to use indicies as normals need to be present for each vertex, 
    // indexing won't allow it http://www.opengl-tutorial.org/intermediate-tutorials/tutorial-9-vbo-indexing/
