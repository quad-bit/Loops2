...
VkAttachmentDescription * Renderer::VulkanInterface::UnwrapAttachmentDesc(const Core::Wrappers::RenderPassAttachmentInfo * renderpassAttachmentList, const uint32_t & attachmentCount)
{
...
VkSubpassDescription * Renderer::VulkanInterface::UnwrapSubpassDesc(const Core::Wrappers::SubpassInfo * subpassList, const uint32_t & subpassCount)
{
...
VkSubpassDependency * Renderer::VulkanInterface::UnwrapSubpassDependency(const Core::Wrappers::SubpassDependency * dependencyList, const uint32_t & dependencyCount)
{
...
VkCommandBufferUsageFlagBits Renderer::VulkanInterface::UnwrapCommandBufferUsage(const Core::Enums::CommandBufferUsage * info)
{
    switch (*info)
    {
...
VkSubmitInfo * Renderer::VulkanInterface::UnwrapSubmitInfo(const Core::Wrappers::SubmitInfo * info)
{
...
VkPipelineStageFlags * Renderer::VulkanInterface::UnwrapStageFlags(const Core::Enums::PipelineStage * pipelineStage)
{
...
VkMemoryPropertyFlags Renderer::VulkanInterface::UnwrapMemoryProperty(const Core::Enums::MemoryType * memType)
{
...
VkBufferUsageFlags Renderer::VulkanInterface::UnwrapBufferUsageFlags(const Core::Enums::BufferType * type)
{
...
Core::Enums::ColorSpace Renderer::VulkanInterface::WrapColorSpace(VkColorSpaceKHR space)
{
    switch (space)
    {
...
Core::Enums::Format Renderer::VulkanInterface::WrapFormat(VkFormat format)
{
    switch (format)
    {
...
Renderer::VulkanInterface::VulkanInterface()
{
}

void Renderer::VulkanInterface::Init()
{
...
void Renderer::VulkanInterface::DeInit()
{
...
*/
    GfxVk::Utility::VulkanManager::GetInstance()->DeInit();
    delete GfxVk::Utility::VulkanManager::GetInstance();
}

Core::Enums::Format Renderer::VulkanInterface::GetWindowSurfaceFormat()
{
    VkSurfaceFormatKHR * vkFormat = GfxVk::Utility::VulkanManager::GetInstance()->GetSurfaceFormat();
    Core::Enums::Format format = WrapFormat(vkFormat->format);
    return format;
}

Core::Enums::ColorSpace Renderer::VulkanInterface::GetWindowColorSpace()
{
    VkSurfaceFormatKHR * vkFormat = GfxVk::Utility::VulkanManager::GetInstance()->GetSurfaceFormat();
    return WrapColorSpace(vkFormat->colorSpace);
}

int Renderer::VulkanInterface::FindBestDepthFormat(Core::Enums::Format * imageFormat, const uint32_t & count)
{
    return GfxVk::Framebuffer::VkAttachmentFactory::GetInstance()->FindBestDepthFormat(imageFormat, count);
}

void Renderer::VulkanInterface::SetupPresentationEngine(Core::Wrappers::ImageInfo info)
{
    GfxVk::Utility::PresentationEngine::GetInstance()->CreateSwapChain(info);
}

void Renderer::VulkanInterface::CreateRenderTarget(Core::Wrappers::ImageInfo * info, Core::Wrappers::ImageViewInfo * viewInfo, uint32_t & count, bool defaultTarget,
    uint32_t * ids)
{
    //VkAttachmentFactory::GetInstance()->CreateColorAttachment(info, count, defaultTarget, ids);
}

void Renderer::VulkanInterface::CreateDefaultRenderTarget(Core::Wrappers::ImageInfo info, Core::Wrappers::ImageViewInfo viewInfo, uint32_t & count, uint32_t * ids)
{
...
void Renderer::VulkanInterface::DestroyRenderTarget(std::vector<uint32_t>* ids, bool defaultTarget)
{
    GfxVk::Framebuffer::VkAttachmentFactory::GetInstance()->DestroyAttachment(*ids, defaultTarget);
}

void Renderer::VulkanInterface::CreateAttachment(Core::Wrappers::ImageInfo * info, const uint32_t & count, uint32_t * ids)
{
...
void Renderer::VulkanInterface::DestroyDepthTarget(std::vector<uint32_t>* ids, bool defaultTarget)
{
    GfxVk::Framebuffer::VkAttachmentFactory::GetInstance()->DestroyAttachment(*ids, defaultTarget);
}

void Renderer::VulkanInterface::DestroyAttachment(uint32_t * ids, bool * destroyImageView, bool * freeImageMemory, const uint32_t & count)
{
    GfxVk::Framebuffer::VkAttachmentFactory::GetInstance()->DestroyAttachment(ids, destroyImageView, freeImageMemory, count);
}

void Renderer::VulkanInterface::FreeAttachmentMemory(uint32_t * imageIds, const uint32_t & count)
{
    GfxVk::Framebuffer::VkAttachmentFactory::GetInstance()->FreeAttachmentMemory(imageIds, count);
}

void Renderer::VulkanInterface::DestroySwapChainImageViews(uint32_t * ids, const uint32_t & count)
{
    GfxVk::Framebuffer::VkAttachmentFactory::GetInstance()->DestroySwapChainImageViews(ids, count);
}

void Renderer::VulkanInterface::CreateImageView(Core::Wrappers::ImageViewInfo * viewInfo, uint32_t & count)
{
    GfxVk::Framebuffer::VkAttachmentFactory::GetInstance()->CreateImageView(viewInfo, count);
}

void Renderer::VulkanInterface::CreateRenderPass(
...
void Renderer::VulkanInterface::DestroyRenderPass(const uint32_t & id)
{
    GfxVk::Renderpass::VkRenderPassFactory::GetInstance()->DestroyRenderPass(id);
}

void Renderer::VulkanInterface::CreateFrameBuffer(uint32_t numFrameBuffers, uint32_t * imageViewId, const uint32_t & viewsPerFB,
...
void Renderer::VulkanInterface::DestroyFrameBuffer(uint32_t * pid, const uint32_t & count)
{
    GfxVk::Framebuffer::VkFrameBufferFactory::GetInstance()->DestroyFrameBuffer(pid, count);
}

void Renderer::VulkanInterface::DestroyCommandBuffer(const uint32_t & id)
{
    GfxVk::CommandPool::VkCommandBufferFactory::GetInstance()->DestroyCommandBuffer(id);
}

void Renderer::VulkanInterface::ResetCommandBuffer(const uint32_t & id, const uint32_t & poolId)
{
    GfxVk::CommandPool::VkCommandBufferFactory::GetInstance()->ResetCommandBuffer(id, poolId);
}

void Renderer::VulkanInterface::BeginCommandBufferRecording(const uint32_t & id, 
...
void Renderer::VulkanInterface::EndCommandBufferRecording(const uint32_t & id)
{
    GfxVk::CommandPool::VkCommandBufferFactory::GetInstance()->EndCommandBufferRecording(id);
}

void Renderer::VulkanInterface::SubmitJob(const Core::Wrappers::QueueWrapper * queueWrapper, const Core::Wrappers::SubmitInfo * info, const uint32_t & submitInfoCount, const uint32_t & fenceId)
{
...
void Renderer::VulkanInterface::SubmitJob(const Core::Wrappers::SubmitInfo * info, const uint32_t & submitInfoCount)
{
    VkSubmitInfo * vkSubmitInfo = UnwrapSubmitInfo(info);
...
void Renderer::VulkanInterface::PresentSwapchainImage(const Core::Wrappers::QueueWrapper * queueWrapper, const Core::Wrappers::PresentInfo * info, const uint32_t & presentQueueId)
{
...
bool Renderer::VulkanInterface::IsApplicationSafeForClosure()
{
    GfxVk::Utility::VkQueueFactory::GetInstance()->WaitForAllQueues();
    vkDeviceWaitIdle(*GfxVk::Utility::CoreObjects::logicalDeviceObj);

    return true;
}

/*
uint32_t * Renderer::VulkanInterface::CreateBuffer(BufferInfo * info, const uint32_t & count)
{
...
//deprecated
uint32_t * Renderer::VulkanInterface::CreateBuffers(Core::Wrappers::BufferInfo * info, const uint32_t & count)
{
    VkBufferUsageFlags * bufferUsage = new VkBufferUsageFlags[count];
    VkMemoryPropertyFlags * memProp = new VkMemoryPropertyFlags[count];
...
void Renderer::VulkanInterface::CreateBuffers(Core::Wrappers::BufferCreateInfo * info, const uint32_t & count, uint32_t * out_buffIds, size_t * out_bufferMemRequirements)
{
    GfxVk::Shading::VkBufferFactory::GetInstance()->CreateBuffers(count, info, out_buffIds, out_bufferMemRequirements);
}

size_t Renderer::VulkanInterface::GetMemoryAlignedDataSizeForBuffer(const size_t & dataSize)
{
    return GfxVk::Shading::VkBufferFactory::GetInstance()->GetMemoryAlignedDataSizeForBuffer(dataSize);
}

uint32_t * Renderer::VulkanInterface::AllocateBufferMemory(uint32_t * bufferId, const uint32_t & bufCount)
{
    return GfxVk::Shading::VkBufferFactory::GetInstance()->AllocateBufferMemory(bufferId, bufCount);
}

void Renderer::VulkanInterface::CopyBufferDataToMemory(const uint32_t & bufId, const VkDeviceSize & dataSize, const VkDeviceSize & memAlignedSize, void * data, VkDeviceSize memoryOffset, bool keepMemoryMounted)
{
...
void Renderer::VulkanInterface::DestroyBuffer(uint32_t * ids, const uint32_t & count)
{
    for (uint32_t i = 0; i < count; i++)
    {
...
void Renderer::VulkanInterface::FreeMemory(uint32_t * ids, const uint32_t & count)
{
    for (uint32_t i = 0; i < count; i++)
    {
...
uint32_t * Renderer::VulkanInterface::AllocateDescriptorsSet(Core::Wrappers::SetWrapper * set, const uint32_t & numDescriptors)
{
    return GfxVk::Shading::VkShaderResourceManager::GetInstance()->AllocateDescriptorSets(set, numDescriptors);
}

void Renderer::VulkanInterface::BindImageMemory(const uint32_t & imageId, const uint32_t & memId, const size_t & offset)
{
    GfxVk::Framebuffer::VkAttachmentFactory::GetInstance()->BindImageMemory(imageId, memId, offset);
}

uint32_t Renderer::VulkanInterface::AllocateMemory(Core::Wrappers::MemoryRequirementInfo * memReq, Core::Enums::MemoryType * userReq, const size_t & allocationSize)
{
...
uint32_t Renderer::VulkanInterface::AllocateMemory(Core::Wrappers::MemoryRequirementInfo * memReq, Core::Enums::MemoryType * userReq, const uint32_t & numUserReq, const size_t & allocationSize)
{
    VkMemoryRequirements req = GfxVk::Unwrap::UnwrapMemoryRequirements(memReq);
...
Core::Wrappers::MemoryRequirementInfo Renderer::VulkanInterface::GetImageMemoryRequirement(const uint32_t & imageId)
{
...
//
//void Renderer::VulkanInterface::InitiateGraphicsPipelineCreation(const uint32_t & meshId, VertexInputAttributeInfo * attribInfo, const uint32_t & attribCount, VertexInputBindingInfo * bindingInfo, const uint32_t & bindingCount )
//{
...
//}
//
//void Renderer::VulkanInterface::SetInputAssemblyInfo(const uint32_t & meshId, PrimtiveType * primitive, bool isPrimitiveRestartEnabled)
//{
//    VulkanGraphicsPipelineFactory::GetInstance()->SetInputAssemblyInfo(meshId, primitive, isPrimitiveRestartEnabled);
//}

void Renderer::VulkanInterface::GetShaderIds(char ** shaderName, Core::Enums::ShaderType * type, uint32_t * id, const uint32_t & shaderCount)
{
    GfxVk::Shading::VkShaderFactory::GetInstance()->GetShaderIds(shaderName, type, id, shaderCount);
}

void Renderer::VulkanInterface::CreateVertexInputState(const Core::Utility::VertexInputWrapper * vertexInputWrapper)
{
    GfxVk::VulkanPipeline::VulkanGraphicsPipelineFactory::GetInstance()->CreateVertexInputState(vertexInputWrapper);
}

void Renderer::VulkanInterface::CreateInputAssemblyState(const Core::Utility::InputAssemblyWrapper * inputAssemblyWrapper)
{
    GfxVk::VulkanPipeline::VulkanGraphicsPipelineFactory::GetInstance()->CreateInputAssemblyState(inputAssemblyWrapper);
}

void Renderer::VulkanInterface::CreateShaderState(const Core::Utility::ShaderStateWrapper * shaderStateWrapper)
{
    GfxVk::VulkanPipeline::VulkanGraphicsPipelineFactory::GetInstance()->CreateShaderState(shaderStateWrapper);
}

void Renderer::VulkanInterface::CreateTessellationState(const Core::Utility::TessellationStateWrapper * wrapper)
{
    GfxVk::VulkanPipeline::VulkanGraphicsPipelineFactory::GetInstance()->CreateTessellationState(wrapper);
}

void Renderer::VulkanInterface::CreateViewportState(const Core::Utility::ViewPortStateWrapper * wrapper)
{
    GfxVk::VulkanPipeline::VulkanGraphicsPipelineFactory::GetInstance()->CreateViewportState(wrapper);
}

void Renderer::VulkanInterface::CreateRasterisationState(const Core::Utility::RasterizationStateWrapper * wrapper)
{
    GfxVk::VulkanPipeline::VulkanGraphicsPipelineFactory::GetInstance()->CreateRasterisationState(wrapper);
}

void Renderer::VulkanInterface::CreateDepthStencilState(const Core::Utility::DepthStencilStateWrapper * wrapper)
{
    GfxVk::VulkanPipeline::VulkanGraphicsPipelineFactory::GetInstance()->CreateDepthStencilState(wrapper);
}

void Renderer::VulkanInterface::CreateColorBlendState(const Core::Utility::ColorBlendStateWrapper * wrapper)
{
    GfxVk::VulkanPipeline::VulkanGraphicsPipelineFactory::GetInstance()->CreateColorBlendState(wrapper);
}

void Renderer::VulkanInterface::CreateMultiSampleState(const Core::Utility::MultiSampleStateWrapper * wrapper)
{
    GfxVk::VulkanPipeline::VulkanGraphicsPipelineFactory::GetInstance()->CreateMultiSampleState(wrapper);
}

void Renderer::VulkanInterface::CreateDynamicState(const Core::Utility::DynamicStateWrapper * wrapper)
{
    GfxVk::VulkanPipeline::VulkanGraphicsPipelineFactory::GetInstance()->CreateDynamicState(wrapper);
}

void Renderer::VulkanInterface::CreatePipeline(Core::Wrappers::PipelineCreateInfo * info, const uint32_t & pipelineCount, uint32_t * pipelineId)
{
    GfxVk::VulkanPipeline::VulkanGraphicsPipelineFactory::GetInstance()->CreatePipeline(info, pipelineCount, pipelineId);
}

std::vector<Core::Wrappers::SetWrapper*> Renderer::VulkanInterface::GetSetsForShaders(const std::vector<std::string>& shaderNames)
{
    return GfxVk::Shading::VkShaderResourceManager::GetInstance()->GetSetsForShaders(shaderNames);
}

uint32_t Renderer::VulkanInterface::CreatePipelineLayout(Core::Wrappers::SetWrapper ** setWrapperList, const size_t & numSets)
{
    return GfxVk::Shading::VkShaderResourceManager::GetInstance()->CreatePipelineLayout(setWrapperList, numSets);
}

std::vector<Core::Wrappers::SetWrapper*>* Renderer::VulkanInterface::GetSetWrapperList()
{
    return GfxVk::Shading::VkShaderResourceManager::GetInstance()->GetSetWrapperList();
}

void Renderer::VulkanInterface::LinkSetBindingToResources(Core::Utility::ShaderBindingDescription * desc, const uint32_t & numBindings)
{
    GfxVk::Shading::VkShaderResourceManager::GetInstance()->LinkSetBindingToResources(desc, numBindings);
}

bool Renderer::VulkanInterface::IsSampleRateShadingAvailable()
{
    return GfxVk::Utility::VulkanManager::GetInstance()->IsSampleRateShadingAvailable();
}

Core::Enums::Samples Renderer::VulkanInterface::GetMaxUsableSampleCount()
{
    return GfxVk::Utility::VulkanManager::GetInstance()->GetMaxUsableSampleCount();
}

uint32_t Renderer::VulkanInterface::CreateSampler(const Core::Wrappers::SamplerCreateInfo & info)
{
    return GfxVk::Shading::VkSamplerFactory::GetInstance()->CreateSampler(info);
}

uint32_t Renderer::VulkanInterface::CreateCommandPool(Core::Enums::PipelineType * pipelineType, Core::Enums::CommandPoolProperty * prop)
{
    uint32_t poolId = 0;
...
void Renderer::VulkanInterface::DestroyCommandPool(const uint32_t & poolId)
{
    GfxVk::CommandPool::VkCommandBufferFactory::GetInstance()->DestroyCommandPool(poolId);
}

uint32_t Renderer::VulkanInterface::CreateFence(bool isSignaled)
{
    return GfxVk::Sync::VkSynchroniserFactory::GetInstance()->CreateFence(isSignaled);
}

uint32_t Renderer::VulkanInterface::Create_Semaphore(bool isSignaled)
{
    return GfxVk::Sync::VkSynchroniserFactory::GetInstance()->Create_Semaphore(isSignaled);
}

void Renderer::VulkanInterface::DestroyFence(const uint32_t & id)
{
    return GfxVk::Sync::VkSynchroniserFactory::GetInstance()->DestroyFence(id);
}

void Renderer::VulkanInterface::DestroySemaphore(const uint32_t & id)
{
    return GfxVk::Sync::VkSynchroniserFactory::GetInstance()->DestroySemaphore(id);
}

void Renderer::VulkanInterface::SetRenderpassBeginInfo(Core::Wrappers::RenderPassBeginInfo * beginInfo, const uint32_t & renderPassId)
{
...
uint32_t Renderer::VulkanInterface::GetAvailableSwapchainIndex(const uint32_t & fenceId, const uint32_t & semaphoreId)
{
    VkFence * fence = GfxVk::Sync::VkSynchroniserFactory::GetInstance()->GetFence(fenceId);
...
void Renderer::VulkanInterface::ActivateCommandBuffer(const uint32_t & index)
{
    GfxVk::CommandPool::VkCommandBufferFactory::GetInstance()->ActivateCommandBuffer(index);
}

GfxVk::CommandPool::VkDrawCommandBuffer * Renderer::VulkanInterface::CreateCommandBuffer(const uint32_t & poolId, uint32_t * cmdBufferId, Core::Enums::CommandBufferLevel* commandBufferLevel, Core::Enums::PipelineType bufferType)
{
